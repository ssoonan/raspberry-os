...
static uint64_t *set_pgd_entry(uint64_t map, uint64_t v, int alloc, uint64_t attribute)
{
    uint64_t *ptr = (uint64_t *)map;
    void *addr = NULL;
    unsigned int index = (v >> 39) & 0x1ff; // PGD 레벨에서 상위 9비트 추출
    ...
    else if (alloc == 1)
    {
        ...
        if (addr != NULL)
        {
            ...
            ptr[index] = (V2P(addr) | attribute | TABLE_ENTRY); // PGD 엔트리에 새 페이지 테이블 설정
        }
    }

    return addr; // 설정된 페이지 테이블의 주소 반환
}



static uint64_t *set_pud_entry(uint64_t *pgd, uint64_t v, int alloc, uint64_t attribute)
{
    uint64_t *ptr = NULL;
    void *addr = NULL;
    unsigned int index = (v >> 30) & 0x1ff; // PUD 레벨에서 9비트 추출
    ...
    else if (alloc == 1)
    {
        ...
        {
            memset(addr, 0, PAGE_SIZE);
            ptr[index] = (V2P(addr) | attribute | TABLE_ENTRY); // PUD 엔트리에 새 페이지 테이블 설정
        }
    }

    return addr; // 설정된 PUD 엔트리의 주소 반환
}


static uint64_t *set_pmd_entry(uint64_t *pud, uint64_t v, int alloc, uint64_t attribute)
{
    uint64_t *ptr = NULL;
    void *addr = NULL;
    unsigned int index = (v >> 21) & 0x1ff; // PMD 레벨에서 9비트 추출
    ...
    else if (alloc == 1)
    {
        ...
        {
            memset(addr, 0, PAGE_SIZE);
            ptr[index] = (V2P(addr) | attribute | TABLE_ENTRY); // PMD 엔트리에 새 페이지 테이블 설정
        }
    }

    return addr; // 설정된 PMD 엔트리의 주소 반환
}
...
bool map_page(uint64_t map, uint64_t v, uint64_t pa, uint64_t attribute)
{
    ...
    uint64_t *pgd = NULL; // PGD의 시작 주소를 저장할 포인터
    uint64_t *pud = NULL; // PUD의 시작 주소를 저장할 포인터
    uint64_t *pmd = NULL; // PMD의 시작 주소를 저장할 포인터
    uint64_t *pte = NULL; // 최종적으로 PTE (Page Table Entry) 엔트리를 저장할 포인터

    bool status = true;
    pgd = set_pgd_entry(map, v, 1, attribute);
    if (pgd == NULL)
        return false;

    pud = set_pud_entry(pgd, v, 1, attribute);
    if (pud == NULL)
        return false;

    pmd = set_pmd_entry(pud, v, 1, attribute);
    if (pmd == NULL)
        return false;

    unsigned int index = (v >> 21) & (0x1ff);
    if ((pmd[index] & ENTRY_V) == 0) {
        pmd[index] = ((pa >> PAGE_SIZE) << PAGE_SIZE | attribute | BLOCK_ENTRY);
    } else {
        status = false;
    }
    return status;
...
bool free_page(uint64_t map, uint64_t vstart)
{
    uint64_t *pgd = NULL;
    uint64_t *pud = NULL;
    uint64_t *pmd = NULL;
    ...
    // 가상 주소가 페이지 크기로 정렬되었는지 확인
    ASSERT(vstart % PAGE_SIZE == 0);

    // PGD 엔트리 가져오기 (할당하지 않음)
    pgd = (uint64_t *)set_pgd_entry(map, vstart, 0, 0);
    if (pgd == NULL)
        return false;

    // PUD 엔트리 가져오기 (할당하지 않음)
    pud = (uint64_t *)set_pud_entry(pgd, vstart, 0, 0);
    if (pud == NULL)
        return false;

    // PMD 엔트리 가져오기 (할당하지 않음)
    pmd = (uint64_t *)set_pmd_entry(pud, vstart, 0, 0);
    if (pmd == NULL)
        return false;
    ...
    // PMD 엔트리가 유효한지 확인
    if (pmd[index] & ENTRY_V)
    {
        // 물리 주소 추출 후 가상 주소로 변환하여 해제
        void *phys_addr = (void *)P2V(PDE_ADDR(pmd[index]));
        kfree(phys_addr);
